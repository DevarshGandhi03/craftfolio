...
export const FileUpload = ({ onChange }) => {
  ...
  const handleFileChange = (newFiles) => {
    ...
    onChange && onChange(selectedFile); // Pass the single file to the callback
  };
  ...
  const { getRootProps, isDragActive } = useDropzone({
    ...
    noClick: true, // Prevent click events when dragging
  });
  ...
  return (
    <div className="w-full" {...getRootProps()}>
      <motion.div
        onClick={handleClick}
        whileHover="animate"
        className="p-6 group/file block rounded-lg cursor-pointer w-full relative overflow-hidden" // Reduced padding here
      >
        <input
          ref={fileInputRef}
          ...
          <div className="relative w-full mt-8 max-w-xl mx-auto">
            ...
            {file ? (
              ...
              <motion.div
                ...
                className={cn(
                  "relative group-hover/file:shadow-2xl z-40 bg-white dark:bg-neutral-900 flex items-center justify-center h-24 mt-4 w-full max-w-[8rem] mx-auto rounded-md", // Reduced height here
